...
#
# **Note**: `self.data` and any indicators wrapped with `self.I` (e.g. `self.sma1`) are NumPy arrays for performance reasons. If you prefer pandas Series or DataFrame objects, use `Strategy.data.<column>.s` or `Strategy.data.df` accessors respectively. You can also construct the series manually, e.g. `pd.Series(self.data.Close, index=self.data.index)`.
...
# method returns a pandas Series of simulation results and statistics associated with our strategy. We see that this simple strategy makes 528% return in the period of 9 years, with maximum drawdown 32%, and with longest drawdown period spanning almost two years ...
#
...
# Strategy optimization managed to up its initial performance _on in-sample data_ by over 50% and beat simple 
