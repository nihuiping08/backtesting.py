...
class _Broker:
    ...
    def new_order(self,
        ...
        # Insert SL/TP/trade-closing orders in-front
        if trade:
    ...
    def _process_orders(self):
        ...
        for order in list(self.orders):  # type: Order
            ...
            # precompute true size in units, accounting for margin and spead/commissions
            size = order.size
            if -1 < size < 1:
...
class Backtest:
    ...
    def _compute_stats(self, broker: _Broker, strategy: Strategy) -> pd.Series:
        ...
        mean_return = returns.prod()**(1/(len(returns) or np.nan))
